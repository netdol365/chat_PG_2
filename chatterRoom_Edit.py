...
class chatterRoomWindow(QDialog,QWidget,form_chatterRoom):
    ip = 'localhost'
    port = 9999
    def __init__(self,id,select):
        super(chatterRoomWindow, self).__init__()
        self.conn_socket = None  #  채팅서버와 연결된 소켓
        self.conn()
        self.select = select
        th2 = threading.Thread(target=self.recvMsg)
        th2.start()
        self.initUi(id, select)
        self.show()
        self.setFixedSize(644, 578)
        self.setId()

    def initUi(self,id, select):
        self.setupUi(self)
        ...
        if select == 0:
            ...
            self.setWindowTitle("채팅방2")
        elif select == 2:
            self.setWindowTitle("채팅방3")
        elif select == 3:
            self.setWindowTitle("채팅방4")
        elif select == 4:
            self.setWindowTitle("채팅방5")


    def conn(self):
        self.conn_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        self.conn_socket.connect((chatterRoomWindow.ip, chatterRoomWindow.port))
