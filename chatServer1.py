...
class ChatServer:
    ...
    def open(self):
        ...
        self.server_socket.bind((ChatServer.ip, self.port))
        self.server_socket.listen()

    def run(self):
        self.open()
        print('chatServer({}) is Running...'.format(self.port))
...
def main():
    chatserver1 = ChatServer(1111)
    chatserver1.run()


    # server = ChatServer(self.port)
    # th1 = threading.Thread(target=server.run())
    # th2 = threading.Thread(target=server.run())
    # th3 = threading.Thread(target=server.run())
    # th4 = threading.Thread(target=server.run())
    # th5 = threading.Thread(target=server.run())
    #
    # th1.start()
    # th2.start()
    # th3.start()
